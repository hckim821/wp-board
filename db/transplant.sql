-- =============================================================================
-- 호스트 DB 이관용 DDL (TRANSPLANT.md §2 에서 사용)
--
-- **자동 생성 파일 — 직접 편집 금지.** 정본은 db/schema.sql 이며,
-- 재생성 방법은 TRANSPLANT.md §2.3 (schema.sql 에서 CREATE DATABASE / USE
-- 두 문장만 제거한 것이 이 파일의 전부다).
--
-- 사용법: 호스트 DB 를 선택(USE)한 상태에서 그대로 실행 → wp_* 테이블 19개 생성.
-- 기존 설치의 업그레이드는 이 파일이 아니라 db/migrations/ 를 번호순 적용.
-- =============================================================================
