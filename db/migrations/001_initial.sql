-- =============================================================================
-- 001_initial.sql — 최초 스키마 (재기준선, plan.md §0 의 2계층 구조)
--
-- 이 파일은 **업그레이드 경로**의 출발점이다. 신규 설치는 `db/schema.sql` 하나로
-- 끝나지만, 이미 돌고 있는 호스트 DB 는 여기서부터 번호순으로 적용한다.
--
-- ## 왜 002 를 남기지 않고 재기준선을 잡았는가
--
-- plan.md §0 은 컨테이너의 이름과 소유 관계를 바꾸고(`wp_work_packages` →
-- `wp_templates`, `maker_id` 제거) 프로젝트 계층 7개 테이블을 새로 들인다.
-- 이것을 003 으로 쓰면 RENAME/DROP/CREATE 가 뒤섞인 거대한 파일이 되는데,
-- **아직 이 스키마를 채택한 호스트가 하나도 없다.** 올려야 할 기존 설치가 없는
-- 마이그레이션을 위해 복잡도를 지고 가는 것은 순손실이다. 그래서 001 을 새 스키마로
-- 다시 쓰고 002(`wp_versions.phase_start_no`)를 그 안에 접어 넣었다.
--
-- 이 판단은 "호스트가 아직 없다" 는 전제에 의존한다. 한 곳이라도 채택한 뒤에는
-- 다시 이렇게 할 수 없고, 그때부터는 번호를 이어 붙이는 수밖에 없다.
--
-- 적용: 호스트 DB 를 선택한 상태에서 이 파일을 그대로 실행한다.
--       (`CREATE DATABASE` / `USE` 는 들어 있지 않다 — 의도적이다.)
-- =============================================================================


-- -----------------------------------------------------------------------------
-- 0. wp_document_types — 문서 기준정보 (전역 스코프, 두 계층 공유)
--
...
CREATE TABLE IF NOT EXISTS `wp_document_types` (
  ...
  `phase_label`   VARCHAR(100) NULL,              -- `Phase 0~1` 등 표시용 자유 텍스트
  `gate_code`     VARCHAR(50)  NULL,              -- `G0·G1`
  `default_owner` VARCHAR(200) NULL,              -- 작성 주체
...
-- document_type_id 는 호스트 문서 마스터로 대체될 경우 **재지정 대상**이다
-- (INTEGRATION.md §3).
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `wp_item_documents` (
  ...
  `sort_order`       INT NOT NULL DEFAULT 0,
  PRIMARY KEY (`item_id`, `document_type_id`),
  KEY `idx_wpid_doc` (`document_type_id`),
  CONSTRAINT `fk_wpid_item`
    FOREIGN KEY (`item_id`) REFERENCES `wp_items` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_wpid_document_type`
...
--
-- **전역 `wp_document_types` 를 그대로 가리킨다.** 프로젝트 생성 시 문서 기준정보는
-- 복제하지 않는다 (plan.md §0.1). 호스트 문서 마스터로 병합할 때 재지정할 FK 가
-- wp_item_documents 와 함께 두 곳이 되는 셈이다 (INTEGRATION.md §3).
-- -----------------------------------------------------------------------------
